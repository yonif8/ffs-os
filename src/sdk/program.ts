...
export const OP = {
  ...
  /** ★ THE NEWGFX FAMILY (0x24–0x27) — now with byte-golden'd emitters here (`Op.font`,
   *  `Op.image`, `Op.anim`, `Op.ink`), so the per-opcode vectors in `tools/ffsp_goldens.json` can
   *  actually see this port. That check is the ONLY thing that catches the drift `ffs_prog.h`'s
   *  header warns CRC-16 cannot: the same tool computes both sides of the CRC. These emitters used
   *  to be absent, and the golden carried entries with no TS counterpart until the cross-check's
   *  missing-key policy went red.
   *
   *    0x24 FONT   (2026-08-13)  sets LV_STYLE_TEXT_FONT on a TEXT slot's label (SMALL ENUM, 0..5)
   *    0x25 IMAGE  (2026-08-13)  common_image_create — an L8 surface the glasses own
   *    0x26 ANIM   (2026-08-13)  four eased lv_anims; replaces the measured-broken BOUNCE
   *    0x27 INK    (2026-08-13)  ★ caller pixels, rasterised ON THE GLASSES
   *
   *  ⚠️ ALL FOUR ARE [M] — mapped, never run on-glass — so an authored art screen still relies on a
   *  flashed interpreter; the emitters exist so the phone CAN author one and its bytes are pinned.
   *
   *    0x28 GIF    (2026-08-14)  ★ NATIVE FRAME CYCLING — HAS an emitter here (`Op.gif`), so
   *                              the phone can drive an animation over slots gen_ffs_os built.
   *                              It carries SLOT INDICES, never pixels — the interpreter builds
   *                              the frame_ptr_array on-glass. ⚠️ [M-FLASH]: never run on
   *                              hardware; a current interpreter SKIPS it (§2) and renders the
   *                              static first frame, so a GIF program is forward-compatible.
   *
   *  Nothing on the wire changes by adding any of these ids — §2's total skippability means an
   *  interpreter that lacks one steps over it. */
  FONT: 0x24, IMAGE: 0x25, ANIM: 0x26, INK: 0x27, GIF: 0x28,
...
/** GIF (0x28) frame-cycling mode — maps 1:1 to the generic_animation instance's +0x19 (RE:
 *  anim_advance). LOOP wraps 0..n-1 forever; ONCE stops on the last frame; PINGPONG bounces. */
export const GIF_MODE = { LOOP: 0, ONCE: 1, PINGPONG: 2 } as const;
const GIF_MODE_MAX = 2;
/** FFSP_GIF_MIN_FRAMES..FFSP_GIF_MAX_FRAMES. One frame is a still IMAGE, not an animation; the
 *  ceiling is the VM slot count (== FFSP_MAX_SLOTS in the interpreter). */
export const GIF_MIN_FRAMES = 2;
export const GIF_MAX_FRAMES = 16;
/** The firmware's own per-frame default when the cfg period word is 0 (FFSP_GIF_DEF_PERIOD_MS). */
export const GIF_DEF_PERIOD_MS = 50;

// ── the newgfx family (0x24–0x27), all [M] — mapped, NEVER RUN ON HARDWARE (ffs_prog.h §3) ──
/** FFSP_FONT_MAX. The six lv_font_t faces are compile-time constants in the interpreter's .rodata;
 *  the wire carries only this SMALL ENUM, never a pointer. ⚠️ ids 3/4/5 are DIGITS ONLY. */
export const FFSP_FONT_MAX = 5;
/** ⚠️ NOT A CLAMP — common_image_create RETURNS NULL outside these, however confidently its own
 *  "clamping to N" log strings say otherwise (ffs_prog.h IMAGE block). 288x144 is the ceiling. */
export const FFSP_IMG_MIN_W = 20;
export const FFSP_IMG_MAX_W = 288;
export const FFSP_IMG_MIN_H = 20;
export const FFSP_IMG_MAX_H = 144;
/** The IMAGE ctor's OWN radius clamp (>=33 -> 32) — a different instruction on a different struct
 *  from TEXT/LIST's FFSP_MAX_RADIUS (10), so both are real (ffs_prog.h FFSP_IMG_MAX_RADIUS). */
export const FFSP_IMG_MAX_RADIUS = 32;
/** INK (0x27) primitive ids — ffs_prog.h FFSP_INK_*. The set that makes an ICON expressible. */
export const INK = {
  CLEAR: 0, SEG: 1, RECT: 2, RING: 3, DISC: 4, TRI: 5, BEZ: 6, TICK: 7, PIP: 8,
} as const;
export const FFSP_INK_MAX_PRIM = 8;
/** ⚠️ `lvl` is a 0..15 GREY INDEX, not an 0..255 alpha — the panel is 4bpp. No hue anywhere. */
export const FFSP_INK_MAX_LVL = 15;
/** A stroke half-width above this is a fill wearing a disguise, and on a see-through emissive
 *  panel a fill occludes the real world (FFSP_INK_MAX_HALFW). */
export const FFSP_INK_MAX_HALFW = 32;
/** ANIM: what the firmware substitutes for duration 0 (expand_animation_play's own default). */
export const ANIM_DEF_MS = 250;
...
const VBIT = {
  ...
  IMAGE_X: 1, IMAGE_Y: 2,
  INK_A: 1, INK_B: 2,
  // ★ ANIM's [V] set is bounded by vmask's own width: declared order is (slot, dur, delay, x0, x1,
  // y0, y1, w0, w1, h0, h1), so the size channels are operand slots 7..10 and a u8 mask cannot
  // address 8..10. The four position operands take bits 3..6; w/h stay immediates.
  ANIM_X0: 3, ANIM_X1: 4, ANIM_Y0: 5, ANIM_Y1: 6,
  WFCREATE_X: 1,
...
export const Op = {
  ...
  /**
   * 0x24 FONT slot:u8, font_id:u8 — LV_STYLE_TEXT_FONT on a TEXT slot's LABEL (handle+0x08),
   * carried as a SMALL ENUM (0..5) so no pointer ever crosses the wire (which is the whole reason
   * FONT exists rather than `STYLE prop 90` — prop 90 is pointer-typed and refused everywhere).
   * ⚠️ ids 3/4/5 are DIGITS ONLY: a title in one renders as silent gaps (the G2 build draws a
   * missing glyph as nothing and advances 4 px), and only the (id, string) PAIR is wrong, which no
   * encoder can catch. [M] — the setter has never been executed on-glass.
   */
  font(slot: number, fontId: number): Ins {
    rng("FONT", "slot", slot, 0, FFSP_MAX_SLOTS - 1);
    rng("FONT", "font_id", fontId, 0, FFSP_FONT_MAX,
      "ffs_prog.h FFSP_FONT_*: 0 default+icons, 1 body, 2 tiny digits, 3/4/5 digits only");
    return ins("FONT", OP.FONT, new ArgWriter("FONT").u8(slot, "slot").u8(fontId, "font_id").done());
  },

  /**
   * 0x25 IMAGE dst:u8, x:i16[V1], y:i16[V2], w:u16, h:u16, bw:u8, bc:u8, rad:u8 —
   * common_image_create, an L8 pixel surface the glasses own.
   * ⚠️ w/h are REFUSED (not clamped) outside 20..288 / 20..144: the ctor's "clamping to" log
   * strings LIE — out of range it does `movs r0,#0; b epilogue` and RETURNS NULL, a silent
   * no-draw. Only x/y are really clamped. `rad` uses the ctor's own >=33->32 bound, not §7's 10.
   * [M] — never executed on-glass, and it LEAKS 72 + w*h B per push (common_image_destroy is [M]).
   */
  image(p: {
    dst: number; x: Num; y: Num; w: number; h: number;
    bw?: number; bc?: number; rad?: number;
  }): Ins {
    rng("IMAGE", "dst", p.dst, 0, FFSP_MAX_SLOTS - 1);
    rngLit("IMAGE", "x", p.x, -0x8000, 0x7fff);
    rngLit("IMAGE", "y", p.y, -0x8000, 0x7fff);
    rng("IMAGE", "w", p.w, FFSP_IMG_MIN_W, FFSP_IMG_MAX_W,
      "⚠️ NOT A CLAMP — common_image_create RETURNS NULL outside this range; 288 is the largest image this firmware builds");
    rng("IMAGE", "h", p.h, FFSP_IMG_MIN_H, FFSP_IMG_MAX_H,
      "⚠️ NOT A CLAMP — the ctor returns NULL outside it; 144 is the ceiling");
    const bw = p.bw ?? FFSP_DEF_BORDER_W, bc = p.bc ?? FFSP_DEF_BORDER_COLOR, rad = p.rad ?? FFSP_DEF_RADIUS;
    rng("IMAGE", "bw", bw, 0, FFSP_MAX_BORDER_W);
    rng("IMAGE", "bc", bc, 0, FFSP_MAX_BORDER_COLOR, "it is a GREY INDEX, not rgb");
    rng("IMAGE", "rad", rad, 0, FFSP_IMG_MAX_RADIUS, "the IMAGE ctor clamps >=33 to 32 — a different bound from TEXT/LIST's 10");
    const o = new Operands(literalVars, "IMAGE");
    const w = new ArgWriter("IMAGE");
    const x = o.n(p.x, VBIT.IMAGE_X, "x"), y = o.n(p.y, VBIT.IMAGE_Y, "y");
    return ins("IMAGE", OP.IMAGE, w
      .u8(p.dst, "dst").i16(x, "x").i16(y, "y").u16(p.w, "w").u16(p.h, "h")
      .u8(bw, "bw").u8(bc, "bc").u8(rad, "rad").done(), o.vmask);
  },

  /**
   * 0x26 ANIM slot:u8, dur_ms:u16, delay_ms:u16, x0:i16[V3], x1:i16[V4], y0:i16[V5], y1:i16[V6],
   * w0,w1,h0,h1:i16 — expand_animation_play: four eased lv_anims (x, y, w, h) from one call, run by
   * the glasses' own engine. This is what "animation that renders NATIVELY" means, and it REPLACES
   * the measured-broken BOUNCE.
   * ★ OPERAND ORDER IS START,END PER CHANNEL (the cfg's own +0x04..+0x20 order), so the interpreter
   * fills the 36-byte cfg with one loop and no permutation. Only x0/x1/y0/y1 are [V] (vmask bits
   * 3..6); w/h are operand slots 7..10, which a u8 vmask cannot reach. dur_ms 0 => the firmware's
   * 250 ms. [M] — never executed on-glass.
   */
  anim(p: {
    slot: number;
    x0: Num; x1: Num; y0: Num; y1: Num;
    w0: number; w1: number; h0: number; h1: number;
    durMs?: number; delayMs?: number;
  }): Ins {
    rng("ANIM", "slot", p.slot, 0, FFSP_MAX_SLOTS - 1);
    const dur = p.durMs ?? ANIM_DEF_MS, delay = p.delayMs ?? 0;
    rng("ANIM", "dur_ms", dur, 0, 0xffff, "u16 ms; 0 => the firmware's 250 ms");
    rng("ANIM", "delay_ms", delay, 0, 0xffff);
    rngLit("ANIM", "x0", p.x0, -0x8000, 0x7fff);
    rngLit("ANIM", "x1", p.x1, -0x8000, 0x7fff);
    rngLit("ANIM", "y0", p.y0, -0x8000, 0x7fff);
    rngLit("ANIM", "y1", p.y1, -0x8000, 0x7fff);
    for (const [n, v] of [["w0", p.w0], ["w1", p.w1], ["h0", p.h0], ["h1", p.h1]] as const) {
      rng("ANIM", n, v, 1, 0x7fff,
        "a size channel: the interpreter floors it at 1 (a zero/negative extent is a widget that " +
        "vanishes, not a transition); w/h cannot be var-bound — their operand slots are 7..10 and a u8 vmask cannot reach 8..10");
    }
    const o = new Operands(literalVars, "ANIM");
    const w = new ArgWriter("ANIM");
    const x0 = o.n(p.x0, VBIT.ANIM_X0, "x0"), x1 = o.n(p.x1, VBIT.ANIM_X1, "x1");
    const y0 = o.n(p.y0, VBIT.ANIM_Y0, "y0"), y1 = o.n(p.y1, VBIT.ANIM_Y1, "y1");
    return ins("ANIM", OP.ANIM, w
      .u8(p.slot, "slot").u16(dur, "dur_ms").u16(delay, "delay_ms")
      .i16(x0, "x0").i16(x1, "x1").i16(y0, "y0").i16(y1, "y1")
      .i16(p.w0, "w0").i16(p.w1, "w1").i16(p.h0, "h0").i16(p.h1, "h1").done(), o.vmask);
  },

  /**
   * 0x27 INK slot:u8, prim:u8, a:i16[V1], b:i16[V2], c,d,e,f:i16, w:u8, lvl:u8 — CALLER PIXELS,
   * rasterised ON THE GLASSES. The phone sends a SHAPE (~19 B), not pixels — the OPPOSITE of the
   * banned pixel-streaming path: the description crosses the link once and the glasses draw it.
   * ⚠️ Coordinates are WHOLE PIXELS on the target IMAGE's own surface (the interpreter scales by 16
   * into ffs_ink.h's subpixel units), and `lvl` is a 0..15 grey index. The commit is DEFERRED to
   * program end (one D-cache clean + lv_image_set_src per dirty slot), so ordering INK before END is
   * all that is required. [M] — never executed on-glass.
   */
  ink(p: {
    slot: number; prim: number;
    a?: Num; b?: Num; c?: number; d?: number; e?: number; f?: number;
    w?: number; lvl?: number;
  }): Ins {
    rng("INK", "slot", p.slot, 0, FFSP_MAX_SLOTS - 1);
    rng("INK", "prim", p.prim, 0, FFSP_INK_MAX_PRIM);
    const av = p.a ?? 0, bv = p.b ?? 0, cv = p.c ?? 0, dv = p.d ?? 0, ev = p.e ?? 0, fv = p.f ?? 0;
    const ww = p.w ?? 1, lvl = p.lvl ?? FFSP_INK_MAX_LVL;
    rngLit("INK", "a", av, -0x8000, 0x7fff);
    rngLit("INK", "b", bv, -0x8000, 0x7fff);
    for (const [n, v] of [["c", cv], ["d", dv], ["e", ev], ["f", fv]] as const) rng("INK", n, v, -0x8000, 0x7fff);
    rng("INK", "w", ww, 0, FFSP_INK_MAX_HALFW, "a stroke half-width above this is a fill wearing a disguise");
    rng("INK", "lvl", lvl, 0, FFSP_INK_MAX_LVL, "⚠️ a 0..15 GREY INDEX, not an 0..255 alpha — the panel is 4bpp");
    const o = new Operands(literalVars, "INK");
    const w = new ArgWriter("INK");
    const al = o.n(av, VBIT.INK_A, "a"), bl = o.n(bv, VBIT.INK_B, "b");
    return ins("INK", OP.INK, w
      .u8(p.slot, "slot").u8(p.prim, "prim")
      .i16(al, "a").i16(bl, "b").i16(cv, "c").i16(dv, "d").i16(ev, "e").i16(fv, "f")
      .u8(ww, "w").u8(lvl, "lvl").done(), o.vmask);
  },

  /**
   * 0x28 GIF dst:u8, period_ms:u16, mode:u8, n:u8, src_slots[n]:u8 — ★ NATIVE FRAME CYCLING.
   *
   * generic_animation_create @0x00463ff8 swaps ONE lv_image's src across the embedded lv_image_dsc
   * (handle+0x24) of `n` EXISTING KIND_IMAGE slots, on a firmware timer with a RESIDENT lv_anim
   * tick driver — so the phone is uninvolved after the push, the OPPOSITE of the banned
   * pixel-streaming path. ⚠️ THE WIRE CARRIES SLOT INDICES, never pixels and never pointers: the
   * interpreter builds the frame_ptr_array on-glass, so FFSP stays pointer-free. Build the frames
   * first (IMAGE + INK, via gen_ffs_os.py) and name their slots here.
   *
   * `n` is DERIVED from `srcSlots.length`, never author-supplied, so it cannot disagree with the
   * trailing count — the same discipline as LIST. There are NO [V] operands (a count and plain slot
   * indices), so vmask is always 0.
   *
   * ⚠️ [M-FLASH] — every firmware call is mapped and NEVER RUN ON HARDWARE; it ships only in a
   * flashed interpreter. On the current blob a GIF is SKIPPED (§2) and the referenced slots render
   * as a static first frame, so a GIF program is forward-compatible. period 0 => the firmware's own
   * 50 ms default. ⛔ It LEAKS ON PURPOSE (leaking beats a use-after-free): the interpreter re-tags
   * the src slots KIND_ANIMFRAME so they are leaked rather than destroyed while the animation still
   * reads them — see ffs_prog.h's GIF block and tools/ffs_gif/.
   */
  gif(dst: number, srcSlots: readonly number[], periodMs = GIF_DEF_PERIOD_MS, mode: number = GIF_MODE.LOOP): Ins {
    rng("GIF", "dst", dst, 0, FFSP_MAX_SLOTS - 1);
    const n = srcSlots.length;
    if (n < GIF_MIN_FRAMES || n > GIF_MAX_FRAMES) {
      throw new Error(
        `GIF: ${n} frame(s); a GIF needs ${GIF_MIN_FRAMES}..${GIF_MAX_FRAMES} src slots ` +
        "(FFSP_GIF_MIN_FRAMES..FFSP_GIF_MAX_FRAMES). One frame is a still IMAGE, not an animation, " +
        `and more than ${GIF_MAX_FRAMES} cannot fit the VM slots.`);
    }
    rng("GIF", "mode", mode, 0, GIF_MODE_MAX, "0 LOOP, 1 ONCE, 2 PINGPONG; maps 1:1 to the instance's +0x19");
    const w = new ArgWriter("GIF");
    w.u8(dst, "dst").u16(periodMs, "period_ms").u8(mode, "mode").u8(n, "n");
    for (let i = 0; i < n; i++) rng("GIF", `src_slots[${i}]`, srcSlots[i], 0, FFSP_MAX_SLOTS - 1);
    for (let i = 0; i < n; i++) w.u8(srcSlots[i], `src_slots[${i}]`);
    return ins("GIF", OP.GIF, w.done());
  },
...
/**
 * ★ THE DISTINCTIVE `code_len` THE ECHO PADS TO — `0x0255` = `0000 0010 0101 0101`.
 *
 * ⚠️ THE ALTERNATING LOW BITS ARE THE POINT, not decoration. `patch_prog.py`'s own docstring says
 * why: "a folded, masked or truncated readback is visible AT A GLANCE rather than needing a diff",
 * and `0x7F000255` as a whole `ret=` word is unmistakable. `echoProgram()` used to be the bare
 * 10-byte EMIT+END, so the expected word was `0x7F00000A` — and 10 is far harder to tell apart from
 * a partially folded or byte-truncated readback than 0x0255 is.
 *
 * ⛔ THIS MUST EQUAL `patch_prog.py::ECHO_LEN_DEFAULT` (0x0255): the whole-program cross-check
 * compares this echo's bytes, header and CRC against the Python golden, and the two must agree on
 * the word CI pushes. TS drifted to 0x0555 once (code_len 1365 vs 597); this is the corrected value.
 */
export const ECHO_LEN_DEFAULT = 0x0255;
