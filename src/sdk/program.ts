...
export const OP = {
  ...
  /** ⛔ THIS PORT IS FOUR OPCODES BEHIND THE CONTRACT, and that is recorded here rather
   *  than discovered later, because it is precisely the drift `ffs_prog.h`'s header warns
   *  CRC-16 cannot catch: the same tool computes both sides of the CRC, so only the
   *  per-opcode byte goldens in `tools/ffsp_goldens.json` can see it — and this file is
   *  supposed to be checked against that exact file.
   *
   *    0x24 FONT   (2026-08-13)  sets LV_STYLE_TEXT_FONT on a TEXT slot's label
   ...
   *
   *  All four are implemented in `patch_prog.py` and in the interpreter, and all four are
   *  pinned by goldens. Nothing on the wire changes by adding them here — they are new ids,
   *  and §2's total skippability means an interpreter that lacks one steps over it. The
   *  cost of the gap is that the PHONE-side SDK cannot yet author a screen that draws its
   *  own art, so anything using them must be generated by tools/gen_ffs_os.py instead. */
  FONT: 0x24, IMAGE: 0x25, ANIM: 0x26, INK: 0x27,
